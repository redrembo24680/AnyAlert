...
def parse_foxtrot_product(self, tracker_id: int, url: str):
    logger.info(f"[FOXTROT] Parsing tracker {tracker_id} for URL: {url}")

    try:
        result = asyncio.run(async_parse_retail(url))
        webhook_url = f"{settings.BACKEND_API_URL}/webhooks/{tracker_id}/foxtrot"
        update_data = {
        ...
        logger.info(
            f"[FOXTROT] Successfully parsed and updated tracker {tracker_id}")
        return result
    except Exception as exc:
        logger.error(
            f"[FOXTROT] Error parsing tracker {tracker_id}: {str(exc)}")
        raise self.retry(exc=exc, countdown=60)


@celery_app.task(bind=True, max_retries=3)
def parse_foxtrot_offers(self, tracker_id: int, url: str):
    logger.info(
        f"[FOXTROT-OFFERS] Parsing offers for tracker {tracker_id} URL: {url}")

    try:
        result = asyncio.run(async_parse_retail(url))
        webhook_url = f"{settings.BACKEND_API_URL}/webhooks/{tracker_id}/foxtrot-offers"
        update_data = {
            "last_price": result.get("price"),
            "last_old_price": result.get("old_price"),
            "last_discount_percent": result.get("discount_percent"),
        ...
        response.raise_for_status()
        logger.info(
            f"[FOXTROT-OFFERS] Successfully parsed offers for tracker {tracker_id}")
        return result
    except Exception as exc:
        logger.error(
            f"[FOXTROT-OFFERS] Error parsing offers for tracker {tracker_id}: {str(exc)}")
        raise self.retry(exc=exc, countdown=60)
