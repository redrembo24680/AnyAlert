...
async def test_parse_foxtrot(url: str):
    print(f"Starting Foxtrot parse: {url}")
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            # Try to find buy button and extract prices from nearby container
            try:
                buy_btn = await page.query_selector('button:has-text("Купити"), button:has-text("Замовити"), button:has-text("Buy")')
                if buy_btn:
            ...
            availability = None
            status = None
            if re.search(r"в\s+наявност|в\s+наличии|in stock|available|є в наявності", body, flags=re.I):
                availability = True
                status = "In stock"
            ...
            reviews_count = None
            reviews_patterns = [
                r'(\d+)\s*(?:відгук|review|comment|rating)',
                r'"(\d+)"',
            ]
...
if __name__ == '__main__':
    url = "https://www.foxtrot.com.ua/uk/shop/smartfoniy-i-mobilniye-telefoniy-samsung-sm-s948b-galaxy-s26-ultra-12256gb-zvd-cobalt-violet.html"
    asyncio.run(test_parse_foxtrot(url))
