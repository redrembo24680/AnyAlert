...
async def test_parse_rozetka(url: str):
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            # Дістаємо стару ціну (якщо є знижка)
            print("Шукаємо стару ціну...")
            old_price = None
            ...
            if old_price_element:
                ...
                print(f"Знайдено текст старої ціни: '{old_price_text}'")

                clean_old_price = re.sub(r'[^\d]', '', old_price_text)
                if clean_old_price:
                    old_price = float(clean_old_price)
                ...
                print("Елемент старої ціни (.product-price__small) не знайдено")
            ...
            if status_element:
                ...
                status = status.strip()
                # Перевіряємо, чи товар у наявності
            ...
            if rating_element:
                ...
                print(f"Знайдено текст рейтингу: '{rating_text}'")

                # Очищаємо рейтинг від "/"
                clean_rating = rating_text.replace('/', '').strip()
                try:
                    rating = float(clean_rating)
                except ValueError:
                    print("Не вдалося перетворити рейтинг на число")
            else:
                print("Елемент рейтингу (span.text-xl.font-bold) не знайдено!")

            print("\n--- РЕЗУЛЬТАТИ ---")
            print(f"Назва: {title}")
            print(f"Поточна ціна: {price} грн")
            print(
